...
>装饰器模式的定义：允许向一个现有的对象添加新的功能，同时又不改变其结构
...
从类图中可以发现一个很特别的点，装饰器类继承了我们被装饰类的同时又持有了被装饰的类。我个人觉得这就是装饰器模式的精髓，利用了组合+继承的方式实现动态扩展。通过继承的方法继承了被装饰类的类型，但是并没有继承被装饰类的行为，而是通过装饰者和组件组合的方式，重新定义了新的行为。
...
通过构造方法传参,我们允许让大剑装饰类允许引用一个角色类，这个角色类不一定就是战士/刺客这种角色实现类哦，还有可能也是一件装备
```java
